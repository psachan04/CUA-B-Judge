...
* **Run Full Pipeline (replay):** `uv run python src/main.py --objective "..." --replay data/replay.json --mode replay`
* **Run Full Pipeline (live):** `uv run python src/main.py --objective "..." --actions data/actions.json --mode live`
* **Run Visual Engine Self-Test:** `uv run python -m src.visual_engine`
* **Run Evaluator Self-Test:** `uv run python -m src.evaluator`
* **Run Trajectory Models Self-Test:** `uv run python -m src.trajectory_models`
* **Compile-Check All Modules:** `for f in src/*.py; do uv run python -m py_compile "$f"; done`
...
### 3. LLM/Inference Layer Constraints
...
### 5. Configuration
* All constants (model slugs, crop sizes, delay timings, marker colors, API endpoints) live in `src/config.py`. Do not hardcode values in other modules.
* The API client (`src/api_client.py`) is the single gateway for all OpenRouter calls. Do not create ad-hoc API clients elsewhere.